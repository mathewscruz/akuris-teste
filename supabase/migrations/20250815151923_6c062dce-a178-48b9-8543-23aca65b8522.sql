...
-- Verificar se a coluna confidencial existe antes de tentar remover
DO $$ 
BEGIN
...
-- Verificar se a coluna categoria existe antes de tentar migrar
DO $$ 
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns 
             WHERE table_name = 'documentos' 
...
-- Criar trigger para notificações se a tabela documentos_aprovacoes existir
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables 
             WHERE table_name = 'documentos_aprovacoes') THEN
